...
contract Option is IOption, StandardToken {
    ...
    function issueOption(uint256 _assetsOffered, uint256 _premium, uint256 _expiry) onlyBuyer public {
        ...
        uint256 assets = _assetsOffered.mul(strikePrice.mul(10));
    ...
    function exerciseOption(uint256 _amount) external returns (bool) {
        ...
        require(IProxy(tokenProxy).distributeStakes(msg.sender, _amount)); //Nikhil: will transfer _amount base tokens to msg.senders account from option dumber
        // Provide allowance to this by the trader
        require(this.transferFrom(msg.sender, optionDumperAddress, _amount));// Nikhil: options asset (tokens) transfered from msg.sender to dumper
